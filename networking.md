...
- ao executar `docker inspect <nome>` veremos que ambos os containers estão no mesmo subnet (uma subdivisão de uma rede maior, criada para segmentar o espaço de endereços IP em grupos menores e facilitar a organização, segurança e eficiência de uma rede) 

- so because we are using the default bridge network both of them should be in the same subnet
- podemos testar `ping <IP>` pra ver a comunicação entre containers 

- ISOLANDO a network do container

- docker network create secutiry-network

- ao executar `docker run -d --name finance --network=secure-network nginx:latest` . Poderemos verificar que ao executar inspect sua network será personalizada nao funcionando o ping posteriormente, neste caso. 